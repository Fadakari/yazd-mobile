...
import AmazingOfferSlider, {
  OfferProduct,
} from "@/components/AmazingOfferSlider";
import CategorySection, { CategoryItem } from "@/components/CategorySection";
import ShegeftAngizSlider, {
  ProductCardType,
} from "@/components/ShegeftAngizSlider";
import LatestBlogSection, {
  BlogPostItem,
} from "@/components/LatestBlogSection";
import NewProductsSlider, {
  NewProductItem,
} from "@/components/NewProductsSlider";
...
type CombinedItem =
  | { type: "slider"; data: ApiSection }
  | { type: "banner"; data: ApiBanner };
...
async function getLatestArticles(): Promise<BlogPostItem[]> {
  try {
    ...
    return data["data"].map((item: any) => ({
      ...
      // اگر عکس نبود، رشته خالی بگذار تا بعداً لوگو جایگزین شود
...
const dummyBlogPosts: BlogPostItem[] = [
  {
    ...
    excerpt:
      "در این مقاله به بررسی مهم‌ترین ویژگی‌های یک پاوربانک خوب می‌پردازیم...",
    image:
      "https://images.unsplash.com/photo-1609592424368-e66a8589c31c?q=80&w=600&auto=format&fit=crop",
    date: "1403/10/05",
    author: "محمد احمدی",
...
// --- تابع کمکی برای نمایش فقط دسته‌بندی‌های اصلی ---
...
async function getActiveLogo(): Promise<string> {
  try {
    const res = await fetch(`${API_URL}/home/active-logo/`, {
...
export default async function Home() {
  ...
  const categoriesTreePromise = GetShopCategoriesTreeList();
  const logoPromise = getActiveLogo();

  const [apiData, fetchedBlogPosts, categoriesTree, siteLogo] =
    await Promise.all([
      homeDataPromise,
      blogDataPromise,
  ...
  const mapApiProductToNewProduct = (p: ApiProduct): NewProductItem => {
  ...
  const shegeftAngizData: ProductCardType[] = (
    firstProductSection?.products || []
  )
    .map(mapApiProductToNewProduct)
    .map((p) => ({
      id: p.id,
      title: p.title,
      price: p.price,
  ...
  const bannerSectionToRender =
    apiData.find((item) => item.item_type === "banner") || null;

  const staticBannerSections = apiData.filter(
    (item) => item.item_type === "static_banner",
  );

  const nonBannerSections = apiData.filter(
    (item) => item.item_type !== "banner" && item.item_type !== "static_banner",
  );
  ...
  return (
    <main className="w-full bg-[#f9f9f9] min-h-screen pb-10">
          ...
          <div>
            <div className="grid grid-cols-12 gap-4 lg:h-[350px]">
              <div className="col-span-12 lg:col-span-9 h-[300px] md:h-[420px] lg:h-full">
                {(() => {
                  const slides: SliderItem[] = (
                    bannerSectionToRender.banner_image || []
                  ).map((b: ApiBanner) => ({
                    id: b.id,
              ...
              <div className="hidden lg:block lg:col-span-3 h-full overflow-hidden">
                {verticalOffers.length > 0 && (
                  <AmazingOfferSlider offers={verticalOffers} />
                )}
              </div>
            </div>
          </div>
          ...
          return combined.map((item) => {
            if (item.type === "slider") {
              ...
              if (section.item_type === "product") {
                const products: NewProductItem[] = (section.products || []).map(
                  mapApiProductToNewProduct,
                );
              ...
              if (section.item_type === "category") {
                const catProductsRaw = (section.categories || []).flatMap(
                  (c) => c.products || [],
                );

                const products: NewProductItem[] = catProductsRaw.map(
                  mapApiProductToNewProduct,
                );
      ...
      {/* شگفت‌انگیز */}
      {shegeftAngizData.length > 0 && (
        <ShegeftAngizSlider products={shegeftAngizData} />
      )}
