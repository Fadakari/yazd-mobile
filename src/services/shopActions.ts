...
export async function GetShopCategoriesTreeList() {
  try {
    ...
    const res = await fetch(`${API_URL}/shop/categories/tree/`, {
      ...defaultFetchOptions,
      next: { revalidate: 30 }, // <--- دیتا کش می‌شود اما هر 60 ثانیه در صورت نیاز آپدیت می‌شود
    });
...
export async function GetProducts(
  ...
  try {
    ...
    if (searchParams?.category_id !== undefined)
      query.append("category_id", searchParams.category_id.toString());
    if (searchParams?.min_price !== undefined)
      query.append("min_price", searchParams.min_price.toString());
    if (searchParams?.max_price !== undefined)
      query.append("max_price", searchParams.max_price.toString());
    if (searchParams?.is_available !== undefined)
      query.append("is_available", String(searchParams.is_available));
    if (searchParams?.is_featured !== undefined)
      query.append("is_featured", String(searchParams.is_featured));
    if (searchParams?.search) query.append("search", searchParams.search);
    if (searchParams?.new_days !== undefined)
      query.append("new_days", searchParams.new_days.toString());
    if (searchParams?.sort) query.append("sort", searchParams.sort);
    if (page) query.append("page", String(page));

    if (onlyDiscounted) {
      // جایگزین api.get با fetch نیتیو
      const resDiscounted = await fetch(
        `${API_URL}/home/discounted-products/?${query.toString()}`,
        {
          ...defaultFetchOptions,
          next: { revalidate: 30 },
        },
...
export async function GetLatestProducts() {
  try {
    ...
    const latestData = resLatest.ok
      ? await resLatest.json()
      : { latest_products: [] };
...
export async function GetProductBySlug(slug: string): Promise<any> {
  try {
    const [productRes, discountRes] = await Promise.allSettled([
      ...
      fetch(
        `${API_URL}/home/discounted-products/${encodeURIComponent(slug)}/`,
        {
          ...defaultFetchOptions,
          next: {
            revalidate: 30,
    ...
    if (productRes.status !== "fulfilled") return null;

    if (!productRes.value.ok) return null;
...
export async function marketing_create_order(
  data: any,
  store_name_english: string,
) {
  try {
    const res = await fetch(
      `${API_URL}/internal-api/marketing/store/${store_name_english}/order/create`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export async function PostComment(
  ...
  const res = await fetch(
    `/internal-api/shop/comments/${String(product_id)}/`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
