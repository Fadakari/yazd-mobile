...
export const GetBlogPosts = async (
  ...
  try {
    ...
    const url = `${API_URL}/blog/posts/?${query.toString()}`;
...
export async function GetLatestBlogPosts() {
  try {
    const res = await fetch(`${API_URL}/blog/posts/latest`, {
      ...defaultFetchOptions,
      next: { revalidate: 30 },
    });
...
> {
  try {
    const res = await fetch(
      `${API_URL}/blog/categories/menu_structure/`,
      {
        ...defaultFetchOptions,
...
export async function SearchBlogs(params: Search): Promise<any> {
  ...
  if (searchParams?.category_id !== undefined)
    query.append("category", searchParams?.category_id.toString());

  try {
    // جایگزین api.get
    const result = await fetch(
      `${API_URL}/blog/posts/search?${query.toString()}`,
      {
        ...defaultFetchOptions,
        next: { revalidate: 30 },
      },
